...
export async function addPending(data) {
  ...
  if (bot?.admin_tg_user_id) {
    ...
    const replyMarkup = {
      inline_keyboard: [
        [
          { text: "核准", callback_data: `approve:${data.chatId}` },
          { text: "拒絕", callback_data: `reject:${data.chatId}` },
        ],
      ],
    };

    await telegramApiService
      .sendMessage(bot.bot_token, bot.admin_tg_user_id, text, {
...
export async function approve({ botId, chatId }) {
  const group = await groupChatRepository.updateStatus({
    botId,
    chatId,
  ...
  if (bot) {
    await telegramApiService
      ...
      .catch((err) =>
        console.warn(`[GroupChat] 發送核准通知失敗: ${err.message}`),
      );
  }
...
export async function reject({ botId, chatId }) {
  ...
  if (bot) {
    ...
    await telegramApiService
      ...
      .catch((err) =>
        console.warn(`[GroupChat] leaveChat 失敗: ${err.message}`),
      );
  }
